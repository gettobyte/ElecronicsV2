...
flexcan_data_info_t tx_info = { .data_length = 6, .msg_id_type =
...
status_t sendEulerOverCan(bno055_euler_t *euler) {
	...
	status_t status = FLEXCAN_DRV_ConfigTxMb(INST_FLEXCAN_CONFIG_1, 0, &tx_info,
			TX_MSG_ID);
	// Send data over CAN
	status_t send_status = FLEXCAN_DRV_SendBlocking(
	INST_FLEXCAN_CONFIG_1, 0,               // Use message buffer 0
...
void printEulerOverUart(bno055_euler_t *euler) {
	...
	status_t status = LPUART_DRV_SendDataBlocking(INST_LPUART_1,
			(const uint8_t*) buffer, strlen(buffer), 1000);
...
int main(void) {
	...
	error = FLEXCAN_DRV_Init(INST_FLEXCAN_CONFIG_1, &flexcanState0,
			&flexcanInitConfig0);
	error = LPI2C_DRV_MasterInit(INST_LPI2C0, &lpi2c0_MasterConfig0,
			&lpi2c1MasterState);
	// 2. Initialize the BNO055 sensor
	error = LPUART_DRV_Init(INST_LPUART_1, &lpuart1state,
			&lpuart_0_InitConfig0);
